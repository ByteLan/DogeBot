...
DogeBot 仓库当前包含一个 Node.js 服务端、一个 Electron 桌面客户端和一个 CLI 远程命令客户端。三者按独立项目管理，各自有自己的 `package.json`、`pnpm-lock.yaml` 和 `node_modules`；服务端负责用户登录、SQLite 数据持久化、飞书机器人绑定，以及为每个已绑定机器人维护独立的飞书长连接；桌面客户端用于登录服务端并管理飞书机器人绑定；CLI 客户端用于向服务端注册远程 slash 命令。
...
- `apps/cli`：CLI 远程命令客户端，默认注册 `/cr`，收到服务端转发后把原始命令文本打印到 stdout，并在断线后自动重连。
...
```bash
cd /Users/bytedance/DogeBot/apps/server
pnpm install
...
```bash
cd /Users/bytedance/DogeBot/apps/desktop
pnpm install
pnpm dev
```
...
```bash
cd /Users/bytedance/DogeBot/apps/cli
pnpm install
pnpm dev -- --username admin --password change-me --command /cr
```

注意：日常不要在根目录执行 `pnpm install`。`apps/server`、`apps/desktop` 和 `apps/cli` 是独立 pnpm 项目，分别在各自目录安装依赖和运行脚本。
...
- 远程命令：登录后的远程客户端可通过 SSE 长连接注册 slash 命令，例如 `/cr`；bot 收到命令后会把原始消息文本转发给同一用户的客户端。
...
所有飞书机器人文字命令的用法统一维护在 [`bot-command.md`](./bot-command.md)，包括 `/users`、`/douyin`、`/set-default`、`/add-cron` 和远程客户端注册的命令。

## 宝塔面板部署

推荐用宝塔的 Node 项目或 PM2 管理器长期运行 `apps/server/dist/src/index.js`。飞书事件接收走服务端主动发起的 WebSocket 长连接，不要求服务器有公网入口；但桌面客户端如果不在同一台机器上，需要能访问服务端 HTTP API。
