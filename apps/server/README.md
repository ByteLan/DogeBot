...
- 如果配置了 bot 默认兜底指令，继续执行默认指令；若原消息或其引用消息中含有 @ 用户，会额外在话题内发送候选人员卡片。卡片会展示这些用户，并支持多选；“加入列表并展示所有”不传 `new n`，“加入列表并展示”会按 `/users @xxx new n` 的方式处理（`n` 为已选人数）。两种加入操作都会撤回候选卡片并发送最终人员卡片。
