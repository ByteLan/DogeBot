...
- `DOGEBOT_FEISHU_MESSAGE_RESOURCE_MAX_BYTES`：图片/表情包复读允许下载的资源大小上限，单位为字节，默认 `4194304`（`4MB`）。只有严格小于该值的资源才会继续下载并复读。
- 图片/表情包复读在完整下载前会先探测资源大小；如果探测不到大小，则边下载边限制。下载后的原始资源会缓存在系统临时目录下的 `dogebot-feishu-image-cache`，例如 macOS 上通常是 `/var/folders/.../T/dogebot-feishu-image-cache`，3 天未命中的缓存会定时清理，命中缓存会刷新时间戳。
...
- 如果是 `/help`、`/users`、`/douyin`、`/set-default`、`/add-cron`、`/reverse`、`/反转`、`/revert`、`/撤回` 等命令，或直接发送包含 `reverse`、`反转`、`翻转`、`镜像`、`对称` 的消息，优先按对应逻辑处理。
- 如果消息里包含 `视频无效` 或 `视频失效` 关键词，会从当前消息（没有则回退到引用消息）中取最后一串大于 10 位的数字作为 `aweme_id`，联网检测该抖音视频是否失效；检测到疑似失效时不会立即删除，而是私聊通知管理员确认，详见「抖音视频失效检测」。
- 如果配置了 bot 默认兜底指令，继续执行默认指令；若原消息或其引用消息中含有 @ 用户，会额外在话题内发送候选人员卡片。卡片会展示这些用户，并支持多选；“加入列表并展示所有”不传 `new n`，“加入列表并展示”会按 `/users @xxx new n` 的方式处理（`n` 为已选人数）。两种加入操作都会撤回候选卡片并发送最终人员卡片。普通群或单聊的非话题消息还会显示“是否发送到群聊”单选项，默认“是”；选择“是”后，首张人员列表卡片会直接发送到当前群聊或单聊，第二张 Markdown 卡片和展示实际表单点击人的人员列表卡片都会按 `/users` 的默认行为回复到首张卡片创建的话题中。
...
- `/help`：返回公开的只读帮助首页，展示当前会话的智能互动、随机生图、抖音订阅、定时任务和兜底 @ 人员收集概览。帮助入口与配置入口分开：命令按基础与媒体、抖音、配置与自动化三组展示，OpenAPI 独立成页；会话配置按智能互动、图片生成、抖音订阅、定时任务和高级设置独立成页并分别保存。
- `/help` 卡片通过回调原地换页，所有群成员都可以公开查看和操作，不限制为卡片发起人。取消抖音订阅和删除定时任务需要先选择，再进入独立确认页确认后执行；其他配置页只有一个主要提交按钮，不会连带修改其他模块。
...
服务端会通过抖音移动端分享页 `https://www.iesdouyin.com/share/video/{aweme_id}`（iPhone UA、8 秒超时）判断视频是否失效：失效视频返回的页面标题会退化为「在抖音记录美好生活」之类的兜底文案，据此区分有效与失效。检测出错（网络异常等）时按「有效」处理，避免误删。检测逻辑纯 Node.js 实现，不依赖任何本地编译组件。

触发方式有两类：

- **关键词手动上报**：消息里包含 `视频无效` 或 `视频失效` 时，从当前消息（没有则回退到引用消息）取最后一串大于 10 位的数字作为 `aweme_id` 进行检测。若疑似失效，不会立即删除，而是在与该 bot `/set-default` 管理员的单聊里发送 `www.douyin.com/video/{aweme_id}`，并在该消息的话题内 `reply_in_thread` 回复一张确认卡片，展示 `aweme_id`、标题、触发群聊 ID、触发人等信息，附「取消 / 删除」两个按钮。只有该管理员可以点击：点「删除」会把该 `aweme_id` 在数据库中标记为删除，点「取消」则不删除；无论点哪个，最终都会撤回这张确认卡片。
- **发送前自动校验**：定时任务、`/set-default` 兜底触发、`/douyin` 命令等所有发送抖音视频链接的入口，都会在发送前校验抽到的 `aweme_id`。若检测到失效，不会在对应群聊发送删除确认卡片，仍然只向管理员私聊上报；随后会重新从数据库抽取新的 `aweme_id` 再次检测，最多重复 5 次或直到找到有效的 `aweme_id` 为止。找到有效的就正常发送，全部失效达到上限时则发送最后一次抽到的 `aweme_id` 拼接的链接。
...
- `/reverse`、`/反转`，或直接发送包含 `reverse`、`反转`、`翻转`、`镜像`、`对称` 关键词的消息：优先读取当前消息里的第一张图片；如果当前消息没有图片，则继续尝试读取该消息引用的上一条消息里的图片或表情包。
- 找到图片或表情包后，服务端会复用现有镜像脚本做一次随机轴向、随机方向的镜像反转；如果当前命令消息在话题里，则直接回复到话题里，否则把结果作为新图片发送到当前会话。
...
- `/byte-style 测试文案` 或 `/字节范 测试文案`：立即把文本渲染成“字节范”图片；如果当前命令消息在话题里，则直接回复到话题里，否则发送到当前会话。
- `/scale-new-heights 测试文案` 或 `/勇攀高峰 测试文案`：立即把文本渲染成“勇攀高峰”图片；如果当前命令消息在话题里，则直接回复到话题里，否则发送到当前会话。
