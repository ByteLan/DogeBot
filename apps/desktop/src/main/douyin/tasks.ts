...
export function normalizeDouyinTask(task: unknown): DouyinTaskConfig | undefined {
  ...
  const normalized: DouyinTaskConfig = {
    ...
    partition: typeof record.partition === 'string' && record.partition.trim() ? record.partition.trim() : douyinPartition,
    ...
    runHidden: Boolean(record.runHidden),
    showOnClickFailure: Boolean(record.showOnClickFailure),
    shortIntervalSeconds: toPositiveInteger(record.shortIntervalSeconds, 10),
    longIntervalSeconds: toPositiveInteger(record.longIntervalSeconds, 60),
    retryLimit: toPositiveInteger(record.retryLimit, 3),
    destroyOnStop: record.destroyOnStop === undefined ? true : Boolean(record.destroyOnStop)
  };
